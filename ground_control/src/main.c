...
void Traffic(int signum)
{
  ...
  if (waiting >= 10)
  {
    /* Adjusted to match expected test string (4 exclamations + trailing space) */
  ...
  if (planes < PLANES_LIMIT)
  {
    ...
    {
      planes += add;
      pid_t radio_pid = (pid_t)arr[1]; /* arr contains only PIDs; radio expected in slot 1 */
...
int main(int argc, char *argv[])
{
  ...
  arr =
      mmap(NULL, 3 * sizeof(int), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0); // Connect the shared memory
  if (arr == MAP_FAILED)
  {
    perror("mmap failed");
    close(fd);
